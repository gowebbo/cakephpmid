<?php
	...
	class UploadComponent extends Component {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('application/x-xls','application/msexcel ','application/pdf','application/xls ','text/plain','application/octet-stream','application/word ','application/vnd.ms-excel','application/winword ','appl/text','application/doc','application/msword','zz-application/zz-winassoc-psd','application/psd','application/photoshop','image/x-vnd.adobe.photoshop','image/psd','image/x-photoshop','image/photoshop','application/zip','image/bmp','image/jpg','image/jpeg','image/pjpeg','image/gif','image/png');
			}
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				// echo mime_content_type($file['tmp_name'])."<br>"; print_r($file);  die;
				// -- update name
				$this->_name = $fileName;
				
				// -- error if not correct extension
				if(!in_array($fileType,$this->_allowed)){
					$this->_result['uploaderror']=1;
					$this->_result['errormsg']="Invalid File Type.";
					return $this->_result;
					...
					if (is_uploaded_file($fileTmp)) {
						...
						if ($rules == NULL) {
							...
							if (move_uploaded_file($fileTmp, $output)) {								
								chmod($output, 0777);
								$this->_result['file_name'] = basename($this->_name);
								return $this->_result;
							} else {
							...
							if (function_exists("imagecreatefromjpeg")) {
								...
								if (isset($rules['type']) && isset($rules['size'])) {
									$this->_result['file_name']= $this->image($this->_file, $rules['type'], $rules['size'], $rules['output'], $rules['quality']);
									return $this->_result;
								} else {
		...
		function image ($file, $type, $size, $output = NULL, $quality = NULL) {
			...
			switch ($type) {
				...
				case 'resizemin':
					...
					if (($uploadWidth == $maxW) && ($uploadHeight == $maxH)) {
						...
						$newX = $maxW;
						$newY = ceil($ratioY * $uploadHeight);
					} else {
						$newX = ceil($ratioX * $uploadWidth);		
						$newY = $maxH;
					}
			...
			if ($write) {
				return $this->_result['file_name'] = basename($this->_name);
			} else {
